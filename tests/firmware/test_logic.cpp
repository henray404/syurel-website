...
static void test_rain_window() {
  ...
  // Pin exactly which bin gets evicted, and exactly when. A known count sits
  // in bin 0; it must survive 59 rotations (still within the last 60 minutes)
  // and be cleared on the 60th, when head_ wraps back around to bin 0.
  RainWindow evict;
  evict.addTips(7);                                   // bin[0] = 7
  for (int i = 0; i < 59; ++i) evict.advanceMinute();
  CHECK_NEAR(evict.mmPerHour(1.0f), 7.0f, 1e-4,
             "bin survives 59 advances -- not yet 60 minutes old");
  evict.advanceMinute();                               // 60th advance wraps head_ to 0
  CHECK_NEAR(evict.mmPerHour(1.0f), 0.0f, 1e-4,
             "bin 0 is the one cleared exactly on the 60th advance");
}
...
static void test_level_fsm() {
  ...
  // Rain strictly between RAIN_WASPADA (10) and RAIN_BAHAYA (30) must land on
  // WASPADA, not skip straight to BAHAYA or fall through to AMAN.
  LevelFsm r2;
  CHECK(r2.update(5, true, 20.0f, 0) == WASPADA, "moderate rain between thresholds escalates to WASPADA only");

  CHECK(strcmp(LevelFsm::name(WASPADA), "WASPADA") == 0, "name mapping");

  // Hysteresis: BAHAYA exits at 55, not at 60 (the enter threshold). A height
  // of 58 sits strictly between the two -- below enter, at/above exit -- and
  // must hold BAHAYA indefinitely. Only dropping below 55 may de-escalate,
  // and that transition lands on WASPADA: the BAHAYA -> WASPADA path.
  LevelFsm m;
  m.update(65, true, 0, 0);                                    // -> BAHAYA
  CHECK(m.update(58, true, 0, 10000) == BAHAYA, "58 is below BAHAYA_ENTER(60) but still holds BAHAYA");
  CHECK(m.update(58, true, 0, 200000) == BAHAYA,
        "58 still holds BAHAYA long after, proving the exit edge is 55, not 60");
  m.update(54, true, 0, 220000);                                // below exit: starts pending de-escalation
  CHECK(m.update(54, true, 0, 280000) == WASPADA,
        "below BAHAYA_EXIT(55) and dwell met: de-escalates to WASPADA (BAHAYA -> WASPADA path)");

  // Boundary values: enter thresholds are strict '>', exit thresholds are '>='.
  // No prior test used exactly 25, 30, 55 or 60.
  LevelFsm b1;
  CHECK(b1.update(30, true, 0, 0) == AMAN,
        "exactly at WASPADA_ENTER(30) does not escalate -- enter is strict '>'");
  LevelFsm b2;
  CHECK(b2.update(60, true, 0, 0) == WASPADA,
        "exactly at BAHAYA_ENTER(60) escalates only to WASPADA, not BAHAYA -- enter is strict '>'");
  LevelFsm b3;
  b3.update(35, true, 0, 0);                                    // -> WASPADA
  CHECK(b3.update(25, true, 0, 100000) == WASPADA,
        "exactly at WASPADA_EXIT(25) still counts as WASPADA -- exit is '>=', not below it");
  LevelFsm b4;
  b4.update(65, true, 0, 0);                                    // -> BAHAYA
  CHECK(b4.update(55, true, 0, 100000) == BAHAYA,
        "exactly at BAHAYA_EXIT(55) still counts as BAHAYA -- exit is '>=', not below it");
  ...
  LevelFsm w;
  w.update(35, true, 0, t1);                                    // -> WASPADA
  w.update(20, true, 0, t1);                                    // below exit: pending=AMAN, since_=t1
  CHECK(w.update(20, true, 0, 10000UL) == WASPADA,
        "30000 ms after since_ (straddling the wrap): dwell not yet met, still holds");
  CHECK(w.update(20, true, 0, 50000UL) == AMAN,
  ...
  LevelFsm cc;
  Level ret = cc.update(65, true, 0, 0);
  ...
  LevelFsm ov;
  ov.update(65, true, 0, ov_base);                                // -> BAHAYA
  ov.update(54, true, 0, ov_base);                                // below exit: pending, since_=ov_base
  CHECK(ov.update(54, true, 0, ov_base + 3000) == BAHAYA,
        "3000 ms elapsed (since_+DWELL_DOWN_MS overflows, but now_ms hasn't): holds BAHAYA");
  CHECK(ov.update(54, true, 0, 59999UL) == WASPADA,
